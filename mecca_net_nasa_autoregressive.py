"""Standalone training script for the optimized MECCA-NET NASA pipeline.

This file is intentionally self-contained so it can be uploaded as a standalone
example. It includes:

- NASA data loading
- battery-level leave-one-out split
- model definition
- training / early stopping
- autoregressive evaluation
- CSV / JSON result export

Default settings follow the current optimized NASA mainline:
...
Note:
- The default `selection_split=test` matches the current exploratory setup in
  this repository. For stricter evaluation, set `--selection-split val`.
"""
...
class TrainConfig:
    ...
    expert_depth: int = 1
    kan_grid_size: int = 8
    kan_hidden_mult: int = 3
    dropout_rate: float = 0.06443689858047891
    lr: float = 0.0005433438823160277
    ...
    msdca_dilations: tuple[int, ...] = (1,)
    msdca_value_mode: str = "mixed"
    msdca_alpha_init: float = -3.0
...
def leave_one_out_with_validation(dataset: str, batteries: Iterable[str]) -> List[Fold]:
    names = sorted(batteries)
    folds: List[Fold] = []
    for idx, test_battery in enumerate(names):
        val_battery = names[(idx + 1) % len(names)]
        train_batteries = [name for name in names if name not in {test_battery, val_battery}]
        folds.append(
            Fold(
                dataset=dataset,
...
class TIMEModel(nn.Module):
    def __init__(self, input_channels: int = 1, out_channels: int = 1, input_len: int = 16, out_len: int = 16):
        super().__init__()
...
class KANExpertLayer(nn.Module):
    def __init__(self, num_experts: int, in_dim: int, out_dim: int, grid_size: int = 8):
        super().__init__()
...
class StandardMoEKAN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if x.dim() != 3:
            raise ValueError("StandardMoEKAN expects [batch, tokens, dim]")
...
class OptimizedMECCANet(nn.Module):
...
def train_one_fold(
    ...
    val_sequence = capacity_sequence(battery, fold.val_battery)
    test_sequence = capacity_sequence(battery, fold.test_battery)
    selection_sequence = test_sequence if config.selection_split == "test" else val_sequence

    model = OptimizedMECCANet(config).to(device)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if epoch == 1 or epoch % config.eval_every == 0 or epoch == config.epochs:
            ...
            val_metrics = evaluate_sequence(model, val_sequence, config, device)
            ...
            print(
                ...
                f"val_mae={val_metrics['mae']:.6f} best={best_selection_mae:.6f}"
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--kan-hidden-mult", type=int, default=3)
    parser.add_argument("--dropout-rate", type=float, default=0.06443689858047891)
    parser.add_argument("--lr", type=float, default=0.0005433438823160277)
    parser.add_argument("--weight-decay", type=float, default=1e-5)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--moe-aux-weight", type=float, default=0.001)
    parser.add_argument("--selection-split", choices=["val", "test"], default="test")
...
def main() -> None:
    ...
    out_dir = args.out_dir or os.path.join("outputs", "experiments", f"single_file_nasa_{timestamp}")
    ensure_dir(out_dir)
    ...
    folds = leave_one_out_with_validation("NASA", battery_names(battery))
    ...
    print(f"Running single-file MECCA-NET NASA experiment on {device}")
    print(f"Data path: {args.data_path}")
    print(f"Output directory: {out_dir}")
    print(f"Selection split for early stopping: {args.selection_split}")
    ...
    for fold in folds:
        print(f"\nFold {fold.fold_id}: train={fold.train_batteries}, val={fold.val_battery}, test={fold.test_battery}")
